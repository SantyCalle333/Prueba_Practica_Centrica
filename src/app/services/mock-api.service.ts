...
export class MockApiService {
  // Base de datos falsa que simula el backend
  ...
  /**
   * Aqui simulo una petición HTTP con 400ms de latencia de red.
   * Filtro los clientes cuyo nombre contenga el término buscado (sin distinción de mayúsculas).
   */
  search(term: string): Observable<Customer[]> {
    const results = this.customers.filter(c =>
      c.name.toLowerCase().includes(term.toLowerCase())
    );
    return of(results).pipe(delay(400)); // simula latencia de red
  }
}
